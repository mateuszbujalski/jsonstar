...
type path_segment = 
    /// x.some_field where x is a record (for DU deps we use typename as autogenerated enum field)
    | RecordField : name:string -> typname:string -> path_segment
    /// Selection of a subschema via 'Case?.v' 
    | Variant : name:string -> path_segment

let rec path_to_string (x : list path_segment) : Tot string = 
    match x with
    | [] -> "Id"
    | Variant n :: r -> "Variant " ^ n ^ " " ^ (path_to_string r)
    | RecordField n tn :: r -> "RecordField " ^ n ^ " " ^ tn ^ " " ^ (path_to_string r)

type record_refinement = 
    // A refinement for an enum field (perhaps inside a nested record)
    | EnumField : p:list path_segment -> ref:enum_refinement -> record_refinement
...
type refinement_type =
    ...
    | RecordRefinement : record_refinement -> refinement_type
